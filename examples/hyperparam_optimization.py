...
param_dist = {"n_epochs": np.linspace(250, 2000, 10, dtype=int),
              "batch_size": np.linspace(64, 512, 6, dtype=int),
              "layers": np.linspace(2, 8, 4, dtype=int),
              "neurons": np.linspace(32, 256, 5, dtype=int)}

model = QuantNNRegressor(quantiles=[0.1, 0.9], layers=4, neurons=128, activation="relu", n_epochs=15)

random_search = RandomizedSearchCV(model, param_distributions=param_dist, n_iter=50, cv=3, n_jobs=-1)
